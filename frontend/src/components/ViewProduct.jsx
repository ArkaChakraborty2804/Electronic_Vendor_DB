          ...
          <table style={{ borderCollapse: 'collapse', border: '1px solid #D1D5DB', width: '100%', textAlign: 'left' }}>
            <thead>
              <tr style={{ background: '#6B46C1', color: 'white' }}>
                ...
                <th style={{ padding: '1rem' }}>Product Type</th>
                <th style={{ padding: '1rem' }}>Product ID</th>
                <th style={{ padding: '1rem' }}>Company ID</th>
                <th style={{ padding: '1rem' }}>Price</th>
              </tr>
            </thead>
            <tbody>
              {Array.isArray(products) && products.map(product => (
                <tr key={product.ProductID} style={{ background: '#F3F4F6', color: '#4A5568' }}>
                  <td style={{ border: '1px solid #D1D5DB', padding: '1rem' }}>{product.ProductName}</td>
                  <td style={{ border: '1px solid #D1D5DB', padding: '1rem' }}>{product.ProductType}</td>
                  <td style={{ border: '1px solid #D1D5DB', padding: '1rem' }}>{product.ProductID}</td>
                  <td style={{ border: '1px solid #D1D5DB', padding: '1rem' }}>{product.CompanyID}</td>
                  <td style={{ border: '1px solid #D1D5DB', padding: '1rem' }}>${product.Price}</td>
                </tr>
              ))}
            </tbody>
          </table>
